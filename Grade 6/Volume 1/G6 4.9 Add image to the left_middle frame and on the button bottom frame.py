# Image.open() – Opens an image file (like .jpg or .png) so Python can read and work with it.
# PhotoImage() – Converts the image into a format that Tkinter/CustomTkinter can 
# display on the screen.

# Task:
# Add images to the three buttons inside the bottom_frame.
# 1. Load three images using CTkImage (example: open_file.png, encrypt.png, decrypt.png).
# 2. Set the size of each image to (25, 25).
# 3. Add one image to each button.
# 4. Keep the button text along with the image.
# 5. Place the buttons inside buttons_container using pack() with side="left" and padx=25.
